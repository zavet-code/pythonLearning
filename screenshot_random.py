...
def check_for_url_of_img(page_url):
    ...
    if 'https://image.prntscr.com/image/' in html_text_url:
        match = re.search('https://image.prntscr.com/image/\S+.\w+', html_text_url)
        return (match[0] if match else 'Not found')
    else:
        match = re.search('https://i.imgur.com/\S+.\w+', html_text_url)
        return (match[0] if match else 'Not found')
...
def download_img_from_url(url):
    if 'Not found' in url:
        ...
        filename = "%s.png"% name
        r = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.131 Safari/537.36'}, allow_redirects=True)
